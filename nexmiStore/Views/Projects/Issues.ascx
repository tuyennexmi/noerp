    ...
    $(function () {
        ...
        $('.task-item').on('mouseover', function () {
            ...
            if (className.indexOf('task-child') == -1) {
                fnLoadIssueDetail($(this).attr('id'));
            }
        });
    });
...
<div class="divActions">
    <table style="width: 100%;">
        ...
        <tr>
            <td>
                <%
                    string functionId = NEXMI.NMConstant.Functions.Issue;
                    //Kiểm tra quyền Insert
                    if (GetPermissions.Get((List<NEXMI.NMPermissionsWSI>)Session["Permissions"], functionId, "Insert"))
                    {
                %>
                <button onclick="javascript:fnShowIssueForm('', '', '<%=projectId%>')" class="button"><%= NEXMI.NMCommon.GetInterface("ADD_NEW", langId) %></button>
                <% 
                    }
                    else
                    {
                ...
                %>
                <button class="button" onclick="javascript:fnPopupStageDialog('', '')">Thêm cột</button>
            </td>
...
<div class="divContent">
    ...
    <table style="height: 102%; background-color: #F2F2F2; color: #333333; margin: -5px;" class="tbIssue" cellpadding="0" cellspacing="0">
        <tr>
            <% 
                            ...
                            <div class="unfold">
                                <table style="width: 100%;">
                                    <tr>
                                        ...
                                        <td style="width: 16px;"><a href="javascript:fnShowIssueForm('', '<%=Item.StageId%>', '<%=projectId%>')"><img alt="" class="img-12px" src="<%=Url.Content("~")%>Content/UI_Images/16Add-icon.png" /></a></td>
                                    </tr>
                                    <tr>
                                        <td colspan="3">
                                            <%Html.RenderAction("IssuesOfStage", "Projects", new { stageId = Item.StageId, projectId = projectId });%>
                                        </td>
                                    </tr>
                                </table>
                            </div>
