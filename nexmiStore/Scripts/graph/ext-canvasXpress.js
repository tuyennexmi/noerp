    ...
    if(this.options && this.options.graphType == 'Network')
    {
      var saveNewView = {
          ...
          },
          {
            ...
            tooltip: 'Save All Changes and Reload the Network',
            disabled: true,
            scope: this,
          ...
          {
            tooltip: 'Zoom <%= NEXMI.NMCommon.GetInterface("PRINT", Session["Lang"].ToString()).Name %>',
        ...
        if(this.clipboard)
        {
          ...
          if(this.clipboard.type == 'nodestyle' && this.clipboard.obj && this.clipboard.obj.node)
            items.push({
              ...
              scope: this,
              handler: function() {
                var ns = this.clipboard.obj.node, copy = {width:1,size:1,height:1,labelSize:1,imagePath:1,shape:1,color:1,hideLabel:1,rotate:1,outline:1,outlineWidth:1,pattern:1,eventless:1,hideTooltip:1,hidden:1,anchor:1};
            ...
            items.push({
              ...
              handler: function() {
                ...
                for(var i = 0; i < n.length; i++)
                {
                  ...
                  var id = n[i].id;
                  delete n[i].id;
                  if(this.cleanNode) this.cleanNode(n[i]);
                ...
                for(var i = 0; i < e.length; i++)
                {
                  ...
                  if(this.cleanEdge) this.cleanEdge(e[i]);
...
//         }, '-');
  ...
  copyObj: function(b, e, type, o) {
    ...
    {
      delete o.id;
      if(this.cleanNode) this.cleanNode(o);
      else delete o.data;
    ...
    else if(type == 'edge')
    {
      delete o.id1;
      delete o.id2;
      if(this.cleanEdge) this.cleanEdge(o);
      else delete o.data;
  ...
  constructor: function(config, extCanvas, callback) {
    ...
    var item1 = [
      // label is required, parent, name, color, size, shape can all be hidden if user wants
      {
        ...
        fieldLabel: 'Label',
        items: [
          { xtype: 'textfield', value: isMulti? null : this.extCanvas.getName(config),
    ...
    var bf = function(b, e, noClose) {
      ...
      for(var i = 0; i < allchanged.length; i++)
      {
        ...
        {
          config = p;
          if(!noClose) noClose = i < allchanged.length - 1;
        }
        // assembl parameters
...
Ext.canvasXpress.edgeDialog = Ext.extend(Ext.Window, {
  ...
  constructor: function(config, dir, extCanvas, config1) {
    ...
    bitems.push({
      ...
      '|',
      {
        text: (config.id1 && config.id2 && !config1? 'Change' : 'Add') + ' Edge',
        handler: bbf
      });
    Ext.apply(this, {
      width: ed.width || 350,
